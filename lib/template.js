class word_card extends HTMLElement {
    constructor() {
        ...
        let kel = this.querySelector("#kel");
        this.querySelector("#n").value = kel.querySelector("span").innerText = value[0];
        kel.style.background = rgb("k", value[0]);
        let k_input = () => {
            this.setAttribute("value", this.querySelector("#n").value);
            kel.querySelector("span").innerText = this.querySelector("#n").value;
            kel.style.background = rgb("k", this.querySelector("#n").value);
            word_value_write(this.getAttribute("word"), "k", Number(this.querySelector("#n").value));
            setTimeout(() => {
                this.focus();
            }, 100);
        };
        this.querySelector("#n").oninput = k_input;
        kel.querySelector("#d").onclick = () => {
            this.querySelector("#n").stepDown();
            kel.querySelector("span").innerText = this.querySelector("#n").value;
            k_input();
        };
        kel.querySelector("#u").onclick = () => {
            this.querySelector("#n").stepUp();
            kel.querySelector("span").innerText = this.querySelector("#n").value;
            k_input();
        };

        let sel = this.querySelector("#sel");
        this.querySelector("#s").value = sel.querySelector("span").innerText = value[1];
        sel.style.background = rgb("s", value[1]);
        let s_input = () => {
            this.setAttribute("value", this.querySelector("#s").value);
            sel.querySelector("span").innerText = this.querySelector("#s").value;
            sel.style.background = rgb("s", this.querySelector("#s").value);
            word_value_write(this.getAttribute("word"), "s", Number(this.querySelector("#s").value));
            setTimeout(() => {
                this.focus();
            }, 100);
        };
        this.querySelector("#s").oninput = s_input;
        sel.querySelector("#d").onclick = () => {
            this.querySelector("#s").stepDown();
            sel.querySelector("span").innerText = this.querySelector("#s").value;
            s_input();
        };
        sel.querySelector("#u").onclick = () => {
            this.querySelector("#s").stepUp();
            sel.querySelector("span").innerText = this.querySelector("#s").value;
            s_input();
        };
